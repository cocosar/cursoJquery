...
(async function load() {
  ...
  const cacheExist = async (category) => {
    ...
    if (cacheList) {
      ...
      const {
        data: { movies: data },
      } = await getData(`${URL_API}list_movies.json?genre=${category}`);
  ...
  $form.addEventListener("submit", async (event) => {
    ...
    try {
      const {
        data: { movies: pelicula },
      } = await getData(
        `${URL_API}list_movies.json?limit=1&query_term=${data.get("name")}`
      );
  ...
  // Lógica para random users en sidebar

  // Fetch data
  const URL_RANDOMUSER = "https://randomuser.me/api/";

  const getDataUser = async (url) => {
    const response = await fetch(url);
    const data = await response.json();
    const dataUser = await data.results[0];
    return dataUser;
  };
  // getDataUser(URL_RANDOMUSER);

  // Creando template para List Item de un user

  const userTemplate = (user) => {
    return `
  <li class="playlistFriends-item">
    <a href="#">
      <img src="${user.picture.thumbnail}" alt="userThumbnail" />
      <span>
  ...
  const addingUserTemplate = async (amount = 8) => {
    for (let i = 0; i < amount; i++) {
      const user = await getDataUser(URL_RANDOMUSER);
      const template = await userTemplate(user);
      const $userContainer = document.getElementById("list-friends-users");
